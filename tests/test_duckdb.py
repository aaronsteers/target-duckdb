"""DuckDB tests (originally imported from the SDK)."""
...
def test_duckdb_discovery(duckdb_sample_tap: SQLTap):
    _discover_and_select_all(duckdb_sample_tap)
    duckdb_sample_tap.sync_all()
    stream = cast(SQLStream, duckdb_sample_tap.streams["main-t1"])
...
def test_duckdb_input_catalog(duckdb_sample_tap: SQLTap):
    duckdb_sample_tap.sync_all()
    stream = cast(SQLStream, duckdb_sample_tap.streams["main-t1"])
...
def test_duckdb_tap_standard_tests(duckdb_sample_tap: SQLTap):
    """Run standard tap tests against Countries tap."""
    tests = get_standard_tap_tests(
        type(duckdb_sample_tap), dict(duckdb_sample_tap.config)
    )
    for test in tests:
        test()
...
def test_sync_duckdb_to_duckdb(
    duckdb_sample_tap: SQLTap, duckdb_sample_target: SQLTarget
):
    """End-to-end-to-end test for DuckDB tap and target.

    Test performs the following actions:

    - Extract sample data from DuckDB tap.
    - Load data to DuckDB target.
    - Extract data again from the target DB using the DuckDB tap.
    ...
    orig_stdout, _, _, _ = tap_to_target_sync_test(
        duckdb_sample_tap, duckdb_sample_target
    )
    orig_stdout.seek(0)
    tapped_config = dict(duckdb_sample_target.config)
    catalog = _get_tap_catalog(DuckDBTap, config=tapped_config, select_all=True)
    tapped_target = DuckDBTap(config=tapped_config, catalog=catalog)
...
def test_duckdb_column_addition(duckdb_sample_target: SQLTarget):
    """End-to-end-to-end test for DuckDB tap and target.
    ...
    target_sync_test(duckdb_sample_target, input=StringIO(tap_output_a), finalize=True)
    target_sync_test(duckdb_sample_target, input=StringIO(tap_output_b), finalize=True)


def test_duckdb_column_morph(duckdb_sample_target: SQLTarget):
    """End-to-end-to-end test for DuckDB tap and target.
    ...
    with pytest.raises(NotImplementedError):
        # DuckDB does not support altering column types.
        target_sync_test(
            duckdb_sample_target, input=StringIO(tap_output_b), finalize=True
        )


def test_duckdb_column_no_morph(duckdb_sample_target: SQLTarget):
    """End-to-end-to-end test for DuckDB tap and target.
